        ...
        ref MultiblockStructure ___ms, ref MultiblockStructure ___msOpp, ref InventoryStoneCoffin ___inv)
    {
        ...
        if (!___processComplete && __instance.IsFull && hasLid && __instance.StructureComplete)
        {
            if (___receivesHeat)
            {
                ...
                var newProgress = progress * (1 / (float)FasterSmeltsModSystem.config.CementationFurnaceSpeedRate);
                ...
                float newTemp = num4 * (1 / (float)FasterSmeltsModSystem.config.CementationFurnaceSpeedRate);
